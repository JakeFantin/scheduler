...
export default function reducer(state, action) {
  switch (action.type) {
    ...
    case SET_INTERVIEW: {
      let dayId = Math.floor(action.id / 5 - 0.01);

      // DETERMINES THE NUMBER OF SPOTS STILL
      const numSpots = action.newState.days[dayId].appointments.reduce(
        (count, appointment) => {
          return !action.newState.appointments[appointment].interview
            ? count + 1
            : count;
        },
        0
